...
//  TicTacToe
//
//  Created by Agustin Ramirez on 2023-08-14.
//
...
class ViewController: UIViewController {
    ...
    @IBAction func boardTapAction(_ sender: UIButton) {
        ...
        if checkForVictory(CRUZ) {
            cruzScore += 1
            resultAlert(title: "Gana X!")
        }
        if checkForVictory(CIRCULO) {
            circuloScore += 1
            resultAlert(title: "Gana O!")
        }
    ...
    func resultAlert(title: String) {
        let message = "\nCirculo " + String(circuloScore) + "\n\nCruz " + String(cruzScore)
